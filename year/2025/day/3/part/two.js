...
const getMaxBank = (bank, remainingDigits) => {
  ...
  const biggestJolt = bank.slice(0, bank.length - (remainingDigits - 1)).reduce((acc, jolt) => acc < jolt ? jolt : acc, 0);
  ...
  return `${biggestJolt}` + getMaxBank(bank.slice(joltIndex + 1), remainingDigits - 1);
};
